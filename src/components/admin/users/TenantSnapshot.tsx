...
export function TenantSnapshot({ financials, blacklistReason }: TenantSnapshotProps) {
    ...
    const isBlacklisted = !!blacklistReason;

    return (
        <div className="space-y-4">
            {isBlacklisted && (
                <div className="bg-destructive/10 border border-destructive/20 p-4 rounded-xl flex items-start gap-3 shadow-sm animate-in fade-in slide-in-from-top-2 duration-300">
                    <AlertCircle className="text-destructive shrink-0 mt-0.5" size={20} />
                    <div>
                        <p className="text-sm font-black text-destructive uppercase tracking-tight">Tenant Blacklisted</p>
                        <p className="text-sm text-destructive/80 font-medium leading-relaxed mt-1">
                            Reason: <span className="underline italic">"{blacklistReason}"</span>
                        </p>
                    </div>
                </div>
            )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4 sticky top-0 z-10 py-2 bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
                ...
                <Card className="shadow-sm">
                    <CardContent className="p-4 flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-base font-black text-foreground">₦{lease?.rentAmount?.toLocaleString() || 0} <span className="text-[10px] font-medium text-foreground/60">/yearly</span></p>
                        </div>
                    </CardContent>
                </Card>
                ...
                <Card className="shadow-sm">
                    <CardContent className="p-4 flex items-center gap-3">
                        ...
                        <div>
                            <p className="text-[11px] uppercase font-extrabold text-foreground tracking-tight">Lease Expiry</p>
                            ...
                            {lease?.endDate && (
                                <p className="text-[9px] text-muted-foreground font-medium">Ends: {format(new Date(lease.endDate), 'MMM dd, yyyy')}</p>
                            )}
                        </div>
                    </CardContent>
                </Card>
